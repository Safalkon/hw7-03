...
resource "yandex_compute_instance" "bastion" {
  ...
  boot_disk {
    initialize_params {
      image_id = data.yandex_compute_image.ubuntu-2404-lts-oslogin.image_id
      type     = "network-hdd"
      size     = 10
    }
  }
...
resource "yandex_compute_instance" "data_mysql" {
  name        = "data-mysql" #Имя ВМ в облачной консоли
  hostname    = "data-mysql" #формирует FDQN имя хоста, без hostname будет сгенрировано случаное имя.
  platform_id = "standard-v3"
  zone        = "ru-central1-b" #зона ВМ должна совпадать с зоной subnet!!!
...
resource "local_file" "inventory" {
  ...
  [dataservers]
  ${yandex_compute_instance.data_mysql.network_interface.0.ip_address}
  [webservers:vars]
  ansible_ssh_common_args='-o ProxyCommand="ssh -p 22 -W %h:%p -q safalkon@${yandex_compute_instance.bastion.network_interface.0.nat_ip_address}"'
  [dataservers:vars]
  ansible_ssh_common_args='-o ProxyCommand="ssh -p 22 -W %h:%p -q safalkon@${yandex_compute_instance.bastion.network_interface.0.nat_ip_address}"'
  XYZ
  filename = "./hosts.ini"
